...
import dummyFeaturePost from './featuredPost.json';
...
export const createSearchUrl = (value: string = '', customPath?: string): string =>
  `${customPath || '/search/label/'}${value}`;

// RESIZE IMAGE FOR POST IMAGE
export const resizeImage = (image: string, width = 400, height = 400, crop = true) => {
...
// GET SOCIAL DATA
// export const getSocial = (type?: 'footer') => {
//   switch (type) {
//     case 'footer':
...
export const mapFeedToFeatureData = (obj: object)  => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const data: any = _.get(obj, 'feed.entry[0]', undefined);
